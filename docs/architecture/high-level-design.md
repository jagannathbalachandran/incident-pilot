...
Replace the static flat files with a **FastAPI-based real-time incident generator** that:
...
- **Request-ID tracing** — every Gradio query and FastAPI call gets a unique ID that flows through all logs via `setLogRecordFactory()` and `contextvars`
...
│                                                                     │
│  ┌──────────────────────────┐                                       │
│  │   FastAPI Generator      │   stdout (JSONL logs)                 │
│  │   (port 5000)            │──────┐                                │
│  │                          │      │                                │
│  │  - /metrics              │      ▼                                │
│  │  - /api/incidents/*      │  ┌──────────────────┐                │
...
|---|---|---|
| **FastAPI Generator** | Tick-driven incident simulator; exposes Prometheus metrics | Python 3.11, FastAPI, `prometheus-client` |
| **Prometheus** | Time-series metrics store; scrapes FastAPI `/metrics` | `prom/prometheus:v2.55.0` |
...
**Decision:** Use Docker `loki` log driver to push FastAPI stdout to Loki

The FastAPI generator writes structured JSONL logs to stdout. The Docker Compose logging driver (`grafana/loki`) picks these up and pushes them to the Loki HTTP API. This avoids needing a dedicated log-shipping agent (like Promtail) while maintaining the JSONL format the existing codebase uses.
...
The `request_id` is generated by a FastAPI middleware handler and stored in a `ContextVar`. It also flows into the log format via `setLogRecordFactory()`, so every line in `docker logs flask-generator` carries `[req=...]`.

The generated application log entries (pushed to Loki) also carry the trigger's `request_id` in their JSON body (stored in `ScenarioState.request_id`), enabling cross-service tracing from API trigger → Docker log → Loki entry.
...
- **Reference corpus** for runbooks and postmortems (which are not generated by the FastAPI service)
...
| Prometheus scrape timeout | FastAPI `/metrics` is synchronous and fast (< 5ms); no timeout expected |
