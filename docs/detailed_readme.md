...
- `pdfplumber` / `python-docx` (text extraction for the latest_runbooks corpus)
- Gradio 4.x (web UI)
...
`synthetic-data/latest_runbooks/` (mixed markdown/PDF/DOCX — dispatched by
file extension) and `synthetic-data/postmorterms/` (markdown), chunks both
with the same format-agnostic `SemanticChunker`, embeds them, and saves the
...
**`synthetic-data/latest_runbooks/`**
...
same `SemanticChunker` as latest_runbooks (not markdown-header splitting) —
...
Purpose: build the vector store from the latest_runbooks and postmortem corpus.
Run manually: `.venv/bin/python src/ingestion.py`
Re-run whenever: a runbook or postmortem file is added, changed,
...
```mermaid
flowchart TD
    subgraph RB["synthetic-data/latest_runbooks/"]
...
### 4.6 Semantic chunking (format-agnostic — shared by latest_runbooks and postmortems)
...
The one chunking function used for **both** latest_runbooks and postmortems.
...
postmortems and latest_runbooks share one chunking function despite having
...
- Input: none (reads from `LATEST_RUNBOOKS_DIR` and `POSTMORTEMS_DIR`)
...
**Step 3 — latest_runbooks corpus**
Iterates `LATEST_RUNBOOKS_DIR.iterdir()` (every file, not a glob of one
...
These three were chosen to each target a different latest_runbooks document
...
three latest_runbooks formats/services currently in the corpus.
...
1. `python src/ingestion.py`
   ...
   - For every FILE (any extension) in `synthetic-data/latest_runbooks/`:
...
**`python-docx`**
Added to support the `.docx` files in `synthetic-data/latest_runbooks/`
